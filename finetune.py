...
def train():
    ...
    parser.add_argument("--model_checkpoint", type=str, default="", help="Path or url of the model checkpoint.")
    ...
    training_args = torch.load(os.path.join(args.model_checkpoint, CONFIG_NAME))

    logger.info("Prepare tokenizer and add fine-tuning tokens")
    tokenizer = OpenAIGPTTokenizer.from_pretrained('openai-gpt')  # Let's use a pre-defined tokenizer
    tokenizer.set_special_tokens(FINETUNING_TOKENS)  # Add two new tokens to the vocabulary (at the end)

    logger.info("Load pretrained model and add fine-tuning token embeddings")
    model = TransformerWithLMHead(training_args)
    state_dict = torch.load(os.path.join(args.model_checkpoint, WEIGHTS_NAME))
    model.load_state_dict(state_dict)
    model.add_new_tokens(len(FINETUNING_TOKENS))  # Add two new tokens to the embeddings (at the end)
